...
const signToken = (id: string, role: string, profilePicture: string, updatedUsername: boolean, firstName : string) => {
    return jwt.sign({ id, role, profilePicture, updatedUsername,firstName }, config.JWT_SECRET, {
...
export const createTokenAndSend = async (user: any, statusCode: number, res: Response, firstLogin?: boolean) => {
    try {
        const token: string = signToken(user.id, user.role, user.avatar, user.userName, user.name);
