...
export async function getSingleRecord<T, L>(model: typeof BaseEntity, query: L, key:any="", isCache:boolean=false, cacheLimit: number = 10*60): Promise<T | any> {
    let data;
    try {
        let redisCacheActive = redisClient.isOpen;
        if (isCache && redisCacheActive) {
...
export async function getAllRecordsWithFilter<T, L>(model: typeof BaseEntity, query: L, key:any = "", isCache:boolean = false, cacheLimit: number = 10*60): Promise<T | any> {
    let data;
    try {
        let redisCacheActive = redisClient.isOpen;
        if (isCache && redisCacheActive){
...
export async function deleteRecords<T, L>(model: typeof BaseEntity, query: L): Promise<T | any> {
    try {
        let data = await model.getRepository().createQueryBuilder().delete().where(query).execute();
        return data;
    } catch (err) {
...
export async function updateRecords<T, L, K, M>(model: typeof BaseEntity, query: any, update: any, upsert: boolean): Promise<T | any> {
    try {
        if (upsert) {
...
export async function getFillteredRecordsWithPagination<T, L, K>(model: any, page: any, query: any, orderBy?: any, select?: any, relations?: any, key: any = "", isCache:boolean = false, cacheLimit: number = 10*60): Promise<T | any> {
    let data;
    try {
        let redisCacheActive = redisClient.isOpen;
        if (isCache && redisCacheActive){
...
export async function getFillteredRecordsWithPaginationNew<T, L, K>(model: any, page: any, query: any, orderBy?: any, select?: any, relations?: any, key: any = "", isCache:boolean = false, cacheLimit: number = 10*60): Promise<T | any> {
    let data;
    try {
        let redisCacheActive = redisClient.isOpen;
