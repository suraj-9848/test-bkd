...
async function applyForJob(token, jobId, resumePath) {
  try {
    ...
    const response = await axios.post(
      `${BASE_URL}/hiring/jobs/${jobId}/apply`,
      form,
      {
        headers: {
          ...form.getHeaders(),
...
async function updateApplicationStatus(token, applicationId, status) {
  try {
    const response = await axios.put(
      `${BASE_URL}/admin/hiring/applications/${applicationId}/status`,
      { status },
      { headers: { Authorization: `Bearer ${token}` } },
    );
    console.log(
...
async function updateJob(token, jobId, updates) {
  try {
    const response = await axios.put(
      `${BASE_URL}/admin/hiring/jobs/${jobId}`,
      updates,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
...
async function runTests() {
  try {
    ...
    try {
      ...
      if (jobId) {
        applicationId = await applyForJob(
          studentToken,
          jobId,
          sampleResumePath,
        );
