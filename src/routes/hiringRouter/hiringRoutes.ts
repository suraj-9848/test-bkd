...
import { validateJobBody, validateApplicationBody } from "../../middleware/hiringMiddleware";
...
hiringAdminRouter.put("/applications/:applicationId/status", updateApplicationStatus);

// User routes
hiringUserRouter.use(authMiddleware);

hiringUserRouter.get("/jobs", getOpenJobs);
hiringUserRouter.post("/jobs/:jobId/apply", upload.single('resume'), validateApplicationBody, applyForJob);
