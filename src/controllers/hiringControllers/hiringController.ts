...
import { JobApplication, ApplicationStatus } from "../../db/mysqlModels/JobApplication";
...
export const createJob = async (req: Request, res: Response) => {
  try {
    ...
    try {
      const organization = await getSingleRecord(Org, { where: { id: org_id } });
      if (!organization) {
      ...
      return res.status(500).json({ 
        ...
        details: "There was an error verifying the organization. Please try again.",
        success: false
      });
    }
    ...
    try {
      ...
      if (!job) {
        return res.status(404).json({
          ...
          details: "The job you're trying to update doesn't exist or has been removed.",
          success: false
        });
      }
      ...
      if (eligibleBranches !== undefined) updateData.eligibleBranches = Array.isArray(eligibleBranches) ? eligibleBranches : [eligibleBranches];
      if (status !== undefined && Object.values(JobStatus).includes(status as JobStatus)) updateData.status = status;
  ...
  try {
    ...
    try {
      ...
      if (!job) {
        return res.status(404).json({
          message: "Job not found",
          details: "The job you're trying to delete doesn't exist or has already been removed.",
          success: false
        });
      }

      // Check if there are any applications
      const applications = await getAllRecordsWithFilter(JobApplication, { where: { job_id: jobId } });
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({
        ...
        details: "There was an error retrieving the job details from the database.",
        success: false
      });
    }
  } catch (error) {
...
export const updateApplicationStatus = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const application = await getSingleRecord(JobApplication, { where: { id: applicationId } });
      if (!application) {
        return res.status(404).json({ 
      ...
      if (application.status === ApplicationStatus.HIRED && status !== ApplicationStatus.HIRED) {
        return res.status(400).json({
      ...
      await updateRecords(JobApplication, { id: applicationId }, { status }, false);
      ...
      return res.status(500).json({
        ...
        details: "There was an error updating the application status in the database.",
        success: false
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({
      message: "Internal server error",
      details: "An unexpected error occurred while updating the application status.",
      success: false
    });
  }
};
...
export const getOpenJobs = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      ...
      details: "There was an error retrieving the job listings. Please try again later.",
      success: false
    });
  }
};
  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        const fileName = s3Service.generateUniqueFileName(req.file.originalname, `user_${userId}`);
        ...
        return res.status(500).json({ 
          ...
          details: "There was an error uploading your resume to storage. Please try again.",
          success: false
        });
      }
    } else {
    ...
    try {
      ...
      return res.status(500).json({ 
        ...
        details: "There was an error saving your application. Please try again.",
        success: false
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({ 
      message: "Internal server error",
      details: "An unexpected error occurred while processing your application.",
      success: false
    });
  }
};
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({
        ...
        details: "There was an error retrieving your applications from the database.",
        success: false
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({
      message: "Internal server error",
      details: "An unexpected error occurred while retrieving your applications.",
      success: false
    });
  }
};
