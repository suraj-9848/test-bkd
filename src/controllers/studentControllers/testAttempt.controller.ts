...
export const getAvailableTests = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: "Failed to fetch available tests" });
  }
};
...
export const startTestAttempt = async (req: Request, res: Response) => {
  try {
    ...
    const existingAttempts = await getAllRecordsWithFilter<TestAttempt, any>(
      TestAttempt,
      {
        where: {
          student: { id: userId },
    ...
    if (test.status === TestStatus.PUBLISHED) {
      ...
      await redisClient.set(`test:${testId}:config`, JSON.stringify(test), {
        EX: 86400,
      });
    }
...
export const getTestQuestions = async (req: Request, res: Response) => {
  try {
    ...
    if (cachedQuestions) {
      ...
      const questionsFromDB = await getAllRecordsWithFilter<Question, any>(
        Question,
        {
      ...
      // Cache questions
      await redisClient.set(questionsCacheKey, JSON.stringify(questions), {
        EX: 86400,
      });
    }
    ...
    if (cachedAnswers) {
      ...
      const answersFromDB = await getAllRecordsWithFilter<TestAnswer, any>(
        TestAnswer,
        {
      ...
      if (answersFromDB && answersFromDB.length > 0) {
        ...
        // Cache answers
        await redisClient.set(answersCacheKey, JSON.stringify(answers), {
          EX: 3600,
        });
      }
    }
    ...
    const remainingTime = Math.max(0, endTime.getTime() - now.getTime()) / 1000; // in seconds
...
export const saveAnswer = async (req: Request, res: Response) => {
  try {
    ...
    const existingAnswer = await getSingleRecord<TestAnswer, any>(TestAnswer, {
      where: {
    ...
    if (existingAnswer) {
      // Always store selectedOptions as array for all question types
      existingAnswer.selectedOptions = Array.isArray(selectedOptions)
        ? selectedOptions
        : selectedOptions
          ? [selectedOptions]
          : [];
      ...
      // Always store selectedOptions as array for all question types
      newAnswer.selectedOptions = Array.isArray(selectedOptions)
        ? selectedOptions
        : selectedOptions
          ? [selectedOptions]
          : [];
      newAnswer.textAnswer = textAnswer || "";
